...
osThreadId_t Ethernet_ServerTaskHandle;//网络通信-服务端
...
osThreadId_t EncoderTaskHandle;//电动机编码器通讯
...
osThreadId_t DriveTaskHandle;//控制驱动器
...
osThreadId_t Motor_driveTaskHandle;//电机加速启动
...
osThreadId_t Pulse_EmissionTaskHandle;//脉冲发射
...
osThreadId_t Extreme_position_DriveTaskHandle;//极限位置控制
...
osThreadId_t Uart3_CommunicateTaskHandle;//版本查询
...
osThreadId_t Iwdg_functionTaskHandle;//看门狗函数
...
osThreadId_t Motor_deceleration_stopsTaskHandle;//电机减速停止
...
osThreadId_t First_power_onTaskHandle;//首次上电处理函数
...
void Uart3_Communicate(void *argument);//版本查询
...
void Drive(void *argument)//控制驱动器
{
  ...
  for(;;)
  {
		...
		if(status==osOK)
		{		
			if(flag_First_power_on<1)//判断是否上电处理
			{
				memset(array_Drive_control, 0, 20);
			}
			else if(flag_First_power_on==1)//判断是否上电处理
			{		
				number_Iwdg=0;	
								...
								case 0x01://原点回归/停止
									if(array_Drive_control[10]==0xFF)//回归
									{
										if(AWAY&&CEASE)//不在原点且停止运动
										{
											...
											if(HAL_GPIO_ReadPin(IN2_GPIO_Port,IN2_Pin)==GPIO_PIN_RESET)//在头部-正向移动
											{
...
void Motor_drive(void *argument)//电机加速启动
{
	...
	__HAL_TIM_CLEAR_IT(&htim8, TIM_IT_UPDATE);//**********************************清除更新中断请求位,防止开启中断就进入中断*******************************
...
//		osDelay(1);	
		...
		{	
			...
			HAL_GPIO_WritePin(TIM8_CH3_GPIO_Port,TIM8_CH3_Pin,(GPIO_PinState)array_Motor_drive_receive[0]);//PC7-电机方向控制(GPIO_PIN_RESET:顺时针，GPIO_PIN_SET:逆时针）
			osDelay(1);	
			DO_MOTION;//PA7-电机使能
			osDelay(1);	
			...
			{
				number_v++;
				number_Reloading=(One_Circle_Distance*1000)/number_v;//v=s/t,s=One_Circle_Distance(单位mm),t=1us*n*Subtraction(单位us)，v=One_Circle_Distance/(1us*n*Subtraction/1000000)=(One_Circle_Distance*1000000)/(1us*n*Subtraction)，自动重装载值n=(One_Circle_Distance*1000000)/(variable_Running_speed_value*Subtraction)，
...
void Encoder(void *argument)//电动机编码器通讯
{
	...
	HAL_TIM_Base_Start_IT(&htim3);                    	 //开启1ms定时器中断
  ...
  for(;;)
  {
		...
		if(flag_Encoder_processing==1)
		{
			...
			motor1.direct = __HAL_TIM_IS_TIM_COUNTING_DOWN(&htim2);//如果向上计数（反转），返回值为0，否则返回值为1
			variable_pluse = COUNTERNUM-(RELOADVALUE)/2;
			motor1.totalCount = variable_pluse + motor1.overflowNum * (int16_t)RELOADVALUE/2;//从开始到现在当前1ms的总脉冲数总脉冲数				
			motor1.speed = (float)(motor1.totalCount - motor1.lastCount) / (4 * MOTOR_SPEED_RERATIO * Subtraction);//算得1ms转了多少圈,除以4是因为4倍频
			motor1.lastCount = motor1.totalCount; //记录这一次的计数值 
			motor1.distance = One_Circle_Distance * motor1.speed;//计算1ms移动距离（单位mm）	
			...
			{
				variable_Position = variable_Position + motor1.distance;	//距原点距离
...
void Motor_deceleration_stops(void *argument)//电机减速停止
{
  ...
  for(;;)
  {
		...
		{	
			...
			{
				number_v--;
				number_Reloading=(One_Circle_Distance*1000)/number_v;
				__HAL_TIM_SET_AUTORELOAD(&htim8,number_Reloading-1);//修改自动重装载值
				__HAL_TIM_SET_COMPARE(&htim8, TIM_CHANNEL_1, (number_Reloading/2)-1);//修改占空比
				HAL_TIM_PWM_Start_IT(&htim8,TIM_CHANNEL_1);//启动TIM8_CH1的PWM波输出（中断）
				osDelay(3);
...
void Iwdg_function(void *argument)//看门狗函数
{
  ...
  {
		osDelay(1);	
		if(number_Iwdg<60)//当等于40时，则说明10min没有上位机控制信号，则单片机重启
		{	
			number_Iwdg++;
...
void First_power_on(void *argument)//首次上电处理函数
{
	uint8_t	array_Zeroing[3]={0};
	uint16_t variable_Zero_speed=20;//20mm/s
  for(;;)
  {
		osDelay(1);
		if(flag_First_power_on==0)
		{
			if(CEASE)
			{
				if(HAL_GPIO_ReadPin(IN2_GPIO_Port,IN2_Pin)==GPIO_PIN_RESET)//在头部-正向移动
				{
					array_Zeroing[0]=GPIO_PIN_SET;
					array_Zeroing[1]=variable_Zero_speed>>8&0xff;
					array_Zeroing[2]=variable_Zero_speed&0xff;
					osMessageQueuePut(Motor_driveQueueHandle,array_Zeroing,1,0);	
				}
				else if(REACH)//在头部-正向移动
				{
					flag_First_power_on=1;
					flag_Zero_regression=1;
				}
				...
				{
					array_Zeroing[0]=GPIO_PIN_RESET;
					array_Zeroing[1]=variable_Zero_speed>>8&0xff;
					array_Zeroing[2]=variable_Zero_speed&0xff;
					osMessageQueuePut(Motor_driveQueueHandle,array_Zeroing,1,0);	
				}
