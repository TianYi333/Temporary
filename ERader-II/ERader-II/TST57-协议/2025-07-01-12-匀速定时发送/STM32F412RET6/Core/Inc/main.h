...
extern int32_t  variable_Calculate_location_upload;//计算位置上传（单位0.001mm）
...
extern uint8_t flag_Running_Speed;//运行速度标志位
extern uint8_t flag_Accelerate_completion;//加速完成标志位
extern uint8_t flag_Position_Interval;//位置1mm间隔标志位
extern uint8_t flag_Direction;//运动方向
